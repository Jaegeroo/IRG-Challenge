...
import type { ClientsT } from "@/lib/types";

export const getClients = async (
  searchQuery: string,
  page: number
): Promise<{ data: ClientsT[]; count: number; message: string }> => {
...
export const createClients = async (
  name: string
): Promise<{ error: boolean; message: string; data: ClientsT | null }> => {
  try {
    const supabase = await createClient();
...
export const updateClient = async (form: { id: string; name: string }) => {
  try {
    const supabase = await createClient();
