...
import { ProjectsT, ProjectStateT, ProjectStoreT } from "@/lib/types";
import { createStore } from "zustand";

export const defaultInitState: ProjectStateT = {
  has_more: true,
  loading: false,
  page: 1,
  items: [],
...
export const createProjectStore = (
  initState: ProjectStateT = defaultInitState
) => {
  return createStore<ProjectStoreT>()((set) => ({
    ...initState,
    ...
    fetch: async (searchQuery: string, page: number) => {
      try {
        const { count, data } = await getProjects(searchQuery, page);
