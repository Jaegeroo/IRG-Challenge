...
import { deleteClientById } from "@/actions/client";
import { Loader2 } from "lucide-react";
import DeleteButton from "@/components/ui/delete-button";

export default function DeleteClientButton({ id }: { id: string }) {
  const { deleteState } = useClientStore((state) => state);
  const [open, setOpen] = useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(false);

  const handleSubmit = async () => {
    setLoading(true);
    const { error, message } = await deleteClientById(id);
    if (error) {
      alert(message);
    } else {
      deleteState(id);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader className="border-b pb-2">
          <DialogTitle className="px-2 pt-4">Delete Client</DialogTitle>
          <DialogDescription />
        </DialogHeader>
